...
kpi_ffrs_list <- readRDS("kpi_ffrs_list.rds")
...
basemaps[["region"]] <-  geojson_read("./phl-admin-psgc-master/phl_regions.geojson", what="sp") %>% 
  ...
  mutate(
    region = case_when(
      region == "AUTONOMOUS REGION IN MUSLIM MINDANAO (ARMM)" ~ "BANGSAMORO AUTONOMOUS REGION IN MUSLIM MINDANAO (BARMM)",
      region == "REGION XIII (Caraga)" ~ "REGION XIII (CARAGA)",
      TRUE ~ region
    )
  )
...
basemaps[["province"]] <-  geojson_read("./phl-admin-psgc-master/phl_provinces_ncr-districts_icc.geojson", what="sp") %>% 
  ...
  filter(
    !psgc_province %in% c("PH150700000", "PH129800000", "PH086403000")
  )

reg_map_data = basemaps[["region"]] %>%
  left_join(kpi_ffrs_list[["region"]] %>%
              select(location, number_farmers),
            by = c("region" = "location")) %>%
  rename(location = region)

prov_map_data = basemaps[["province"]] %>%
  left_join(kpi_ffrs_list[["province"]] %>%
              select(location, number_farmers),
            by = c("province" = "location")) %>%
  rename(location = province)

natl_map_data <- reg_map_data |> 
  st_drop_geometry() |> 
  summarise(number_farmers = sum(number_farmers, na.rm = TRUE)) |> 
  mutate(
    number_farmers = format(
      number_farmers, scientific = FALSE, big.mark = ","
    )
  ) |> 
  pull(number_farmers)
```
...
::: {.custom_dt_wrapper}
```{r}
#| label: farmers_count_dt

table_data <- kpi_ffrs_list[["region"]] |> 
  select(location, number_farmers) |> 
  mutate(
    location = case_when(
      location == "BANGSAMORO AUTONOMOUS REGION IN MUSLIM MINDANAO (BARMM)" ~ "BARMM",
      TRUE ~ location
    )
  ) |> 
  arrange(desc(number_farmers))

table_data %>%
  datatable(
    selection = 'single',
    class = "row-border",
    colnames = c("Region", "Number of Farmers"),
    options = list(
      dom = "t",
      scrollX = TRUE,
      pageLength = 50,
      ...
      columnDefs = list(
        ...
        list(targets = c(1), className = "dt-left"),
        list(targets = c(2), className = "dt-right"),
  ...
  formatStyle(
    "number_farmers",
    background = styleColorBar(table_data$number_farmers, "#54FF9F")
  ) %>%
  formatRound(2)
```
:::
...
::: {.column width="60%"}
```{r}
#| label: farmers_count_map
#| echo: false

pal <- colorBin(
  palette = c("#8B1A1A", "#E69F00", "#F0E442", "#117733", "#56B4E9"),
  domain = reg_map_data[["number_farmers"]],
  bins = c(0, 50000, 150000, 300000, 450000, Inf),
  na.color = "transparent"
)

reg_map_data_pal <- reg_map_data |> 
  mutate(
    color = if_else(
      number_farmers == 0, "grey",
      pal(number_farmers)
    )
  )
...
leaflet(
  ...
  addProviderTiles("CartoDB.VoyagerNoLabels", options = providerTileOptions(minZoom = 5)) |>
  ...
  addEasyButton(
    easyButton(
      ...
      title = "Reset Zoom",
      position = "topleft",
      onClick = JS("function(btn, map){ map.setView([12.8797, 121.7740], 6); }")
    )
  ) |>
  addEasyButton(easyButton(
    icon = "fa-minus",
  ...
  addPolygons(
    ...
    label = ~ paste(
      ...
      "<br>",
      "Number of Farmers: ",
      format(round(number_farmers, 2), big.mark = ",")
  ...
  addControl(html = "<div style='font-size: 9px; font-style: italic;'>Source: FFRS</div>", position = "bottomleft")
```
:::
:::
